...
define('API_BASE', 'http://localhost/cursos-joaoribeiro/api/aula_06/api/?option=');

echo '<pre>APLICAÇÃO</pre>';

for($i=0; $i<10; $i++){
    // $resultado = api_request('random'); // var recebe a resposta da funcao abaixo (function api_request())
    //* $resultado = api_request('random&min=1000&max=200'); // emite msg de erro pois MIN nao pode ser maior que MAX (tratado em api)
    // $resultado = api_request('random&min=100&max=200'); // usado quando switch case = random
    $resultado = api_request('hash'); // usado quando switch case = hash
...
//* BLOCO ABAIXO PARA TESTAR A API - BEGIN ------
// echo '<pre>';
// print_r(api_request('status'));
//* BLOCO ABAIXO PARA TESTAR A API - END --------
